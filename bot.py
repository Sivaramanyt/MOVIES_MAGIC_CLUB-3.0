...
def safe_duplicate_policy(data: dict) -> bool:
    """Require filename, byte size and quality to all be present for a safe match."""
...
async def find_safe_duplicate(data: dict):
    if not safe_duplicate_policy(data):
        return None
    return await db.find_duplicate_file(
        data["file_name"], int(data["size"]), data.get("quality"), data.get("file_unique_id")
    )
...
async def reindex_channel(progress_callback=None, delete_duplicates=False) -> dict:
    """Scan channel history. Duplicate deletion is opt-in for safety."""
        ...
        try:
            ...
            if is_other_record:
                if delete_duplicates:
                    ...
                    continue
                # Dry-run: report the candidate but do not delete the message or DB record.
                stats["duplicates"] += 1
...
def format_reindex_progress(stats: dict, started_at: float, running: bool = True, dry_run: bool = True) -> str:
    ...
    return (f"{state}\n{mode}\n\n📦 Scanned: `{stats['scanned']}`\n🗂 Indexed: `{stats['indexed']}`\n"
            f"🧩 Enriched: `{stats['enriched']}`\n♻️ Duplicate candidates: `{stats.get('duplicates', 0)}`\n"
...
async def help_cmd(_, message):
    await message.reply_text("🔎 Send a movie/series name to search.\n\nAdmins: `/stats`\n🔄 `/reindex` = dry run\n🗑️ `/reindex --delete` = delete confirmed duplicates", parse_mode=ParseMode.MARKDOWN)
...
async def reindex_cmd(_, message):
    ...
    if delete_requested and REINDEX_DRY_RUN:
        await message.reply_text("🛡️ REINDEX_DRY_RUN is enabled, so deletion is blocked. Set `REINDEX_DRY_RUN=false` before using `/reindex --delete`.", parse_mode=ParseMode.MARKDOWN)
        return
...
async def page_callback(_, query: CallbackQuery):
    ...
    page = max(0, int(page)); total = await db.count_grouped_movies(search); page = min(page, page_count(total) - 1)
...
async def file_callback(_, query: CallbackQuery):
    ...
    await query.answer("Sending file…"); await app.send_cached_media(query.from_user.id, file_id, caption=file_card(item))
...
async def main():
    await db.setup(); await app.start(); me = await app.get_me(); print(f"@{me.username} is running"); await asyncio.Event().wait()
